// ============================================================
// 分析接口配置
// 填入真实接口后会调用后端/AI；未配置 API Key 时自动走本地 mock，方便 demo 演示。
...
export const MODEL_API_CONFIG = {
  ...
  /**
   * 系统提示词：只输出娱乐化、陪伴式 JSON，不做医疗判断。
   */
  systemPrompt:
    '你是“掉了么”的趣味头发记录陪伴员。用户会上传掉发或头发状态照片。' +
    '请只基于画面给出轻松、娱乐化、非医学的反馈，不要使用诊断、病变、严重风险、治疗等表达。' +
    '请返回严格 JSON，不要返回其他内容：\n' +
    '{\n' +
    ...
    '  "tags": ["趣味标签1", "趣味标签2"],\n' +
    '  "daily_task": "今日建议任务",\n' +
    '  "disclaimer": "本结果仅用于轻松记录和娱乐反馈，不构成医学诊断或治疗建议。",\n' +
    '  "count": "少量|中等|偏多",\n' +
    '  "thickness": "粗硬|正常|细软",\n' +
