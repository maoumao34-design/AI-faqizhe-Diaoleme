...
function attachChatAssistant(root: HTMLElement) {
  ...
  const onSubmit = async (event: SubmitEvent) => {
    ...
    renderMessages()
    try {
      // AIFA-52: attach local Journey history (reportHistory), not shared GET /api/records.
      // AIFA-47: chatWithAssistant itself strips assistant turns (incl. local greeting).
      const report_context = buildReportContext(useUserStore.getState().reportHistory, 5)
      const result = await chatWithAssistant(
        messages.filter((m) => !(m.role === 'assistant' && m.content === thinkingPlaceholder)).slice(-8),
        report_context,
      )
